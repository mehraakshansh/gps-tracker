...
Deno.serve(async(req)=>{
  ...
  if(req.method==="POST"){
    ...
    const {data,error}=await sb.from("zones").insert(body).select().single();
  ...
  if(req.method==="PUT"&&id){
    const body=await req.json();
    const {data}=await sb.from("zones").update(body).eq("id",id).select().single();
    return new Response(JSON.stringify(data),{headers:{...cors,"Content-Type":"application/json"}});
  }
  const {data}=await sb.from("zones").select("*").order("created_at",{ascending:false});
  return new Response(JSON.stringify(data||[]),{headers:{...cors,"Content-Type":"application/json"}});
});
